...
export default function PendingVerifiers(){
  ...
  const fetchList = async()=>{
    setLoading(true);
    try{
      const res = await axios.get(`http://localhost:3500/superadmin/pending-verifiers?page=1&limit=25`);
  ...
  const review = async(id,status)=>{
    try{
      await axios.put(`http://localhost:3500/superadmin/review-verifier/${id}`,{ status });
      // remove from list
      setList(prev=>prev.filter(d=>d._id!==id));
      const successMsg = status === 'approved' ? 'Verifier approved successfully' : 'Verifier rejected';
      setMsg(successMsg);
      setMsgType('success');
  ...
  {msg && <div style={{color: msgType === 'success' ? '#00e676' : 'salmon'}}>{msg}</div>}
      ...
      {list.map(d=> (
        <Row key={d._id}>
          <Info>
            <div style={{fontWeight:700}}>{d.Inititutename || d.institutionName || d.name}</div>
